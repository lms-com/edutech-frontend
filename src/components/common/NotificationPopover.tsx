  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="relative p-2 rounded-xl text-slate-600 hover:text-[#2c3e50] hover:bg-slate-100 transition-colors cursor-pointer"
        title="Thông báo"
      >
        ...
        <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-150">
          <div className="p-4 bg-[#2c3e50] text-white flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-sm">Thông Báo Của Bạn</h3>
                {unreadCount > 0 && (
                  <span className="px-1.5 py-0.2 rounded-full bg-[#e74c3c] text-white text-[10px] font-bold">
                    {unreadCount} mới
                  </span>
                )}
              </div>
              <p className="text-[11px] text-slate-300 mt-0.5">
                Cập nhật tự động các hoạt động học tập & tài khoản
              </p>
            </div>

            <button
              onClick={() => setIsOpen(false)}
              className="p-1 rounded-lg text-slate-300 hover:text-white hover:bg-slate-700/50 cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="p-2.5 bg-slate-50 border-t border-slate-200 text-center">
            <span className="text-[11px] font-medium text-slate-500">
              Thông báo được cập nhật tự động theo thời gian thực
            </span>
          </div>
        </div>
      )}
